<footer class="footer_01 fm_dark">
    <div class="layer_img move_anim_two">
        <img src="{{ asset('makeover/images/bg/footer.png') }}" alt="Footer Image" />
    </div>
    <div class="container">
        <div class="row">
            <div class="col-md-6 offset-md-3 col-lg-6 offset-lg-3 col-xl-4 offset-xl-4">
                <aside class="widget mailchimp">
                    <h3 class="widget_title">Lalisa Spa</h3>
                    <form action="#" method="post">
                        <div class="mc4wp-form-fields">
                            <input type="email" name="EMAIL" placeholder="Enter your email">
                            <button type="submit"><i class="icofont-long-arrow-right"></i></button>
                        </div>
                    </form>
                </aside>
            </div>
        </div>
        <div class="row">
            <div class="col-md-4 col-lg-4 col-xl-4">
                <aside class="widget about_widget">
                    <h3 class="widget_title">Về chúng tôi</h3>
                    <p>
                        Chào mừng bạn đến với Lalisa, trung tâm chăm sóc sức khỏe và làm đẹp hàng đầu, nơi chúng tôi cam
                        kết mang đến cho bạn những trải nghiệm tuyệt vời nhất trong việc chăm sóc bản thân.
                    </p>

                </aside>
            </div>
            <div class="col-md-4 col-lg-4 col-xl-3 offset-xl-1">
                <aside class="widget">
                    <h3 class="widget_title">Liên kết</h3>
                    <ul>
                        <li><a href="{{ route('booking.index') }}">Book Online</a></li>
                        <li><a href="{{ route('products') }}">Sản phẩm của chúng tôi</a></li>
                        <li><a href="{{ route('blog') }}">Các bài viết</a></li>
                    </ul>
                </aside>
            </div>
            <div class="col-md-4 col-lg-4 col-xl-3 offset-xl-1">
                <aside class="widget">
                    <h3 class="widget_title">Liên hệ</h3>
                    <div class="info_box">
                        <i class="icofont-location-pin"></i>
                        <p>Tân Chánh Hiệp,<br> Quận 12, Tp Hồ Chí Minh</p>
                    </div>
                    <div class="info_box">
                        <i class="icofont-phone"></i>
                        <p>Hotline 24/7<br> +84 365335832</p>
                    </div>
                    <div class="info_box">
                        <i class="icofont-clock-time"></i>
                        <p>Thứ 2 đến thứ 6<br> 9:00 am to 9:00 pm</p>
                    </div>
                </aside>
            </div>
        </div>
    </div>
</footer>
<section class="copyright copy_dark">
    <div class="container">
        <div class="row">
            <div class="col-md-6">
                <p>© 2024 <a href="http://themewar.com" target="_blank" rel="noopener">Lalisa</a> </p>
            </div>
            <div class="col-md-6">
                <div class="copy_social">
                    <a target="_blank" href="https://www.facebook.com/"><i class="icofont-facebook"></i></a>
                    <a target="_blank" href="https://twitter.com/"><i class="icofont-twitter"></i></a>
                    <a target="_blank" href="https://bebo.com/"><i class="icofont-bebo"></i></a>
                    <a target="_blank" href="https://dribbble.com/"><i class="icofont-dribbble"></i></a>
                </div>
            </div>
        </div>
    </div>
</section>
<!-- End:: Footer Section -->

<!-- Bact To Top -->
<a href="javascript:void(0);" id="backtotop"><i class="icofont-bubble-up"></i></a>
<!-- Bact To Top -->